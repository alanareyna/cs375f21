...
public:
   ...
   inline void resize(const unsigned int width, const unsigned int height)
   {
      ...
      unsigned int row_inc = width * bytes_per_pixel_;
      temp.resize(height * row_inc);
      std::fill(temp.begin(), temp.end(), clear_val_);
      unsigned w = width < width_ ? width : width_;
      unsigned h = height < height_ ? height : height_;
      // copy old data to temp
      width_ = width;
      height_ = height;
      row_increment_ = row_inc;
      data_.clear();
      data_ = temp;
   }
