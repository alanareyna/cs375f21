...
public class DesignElement
{
	...
	public void rotate (double mouseX1, double mouseY1, double mouseX2, double mouseY2)
	{
		...
		double r1 = Math.atan2(mouseY1 - centerY, mouseX1 - centerX);
		double r2 = Math.atan2(mouseY2 - centerY, mouseX2 - centerX);
		deltaR = r2 - r1;
		rotate(deltaR);
  	}
	...
	{
		double scaleVal;
		Rectangle2D outOfBounds = path.getBounds2D();
		if (Math.abs(outOfBounds.getWidth() * scaleX) < 5)
			return;
		if (Math.abs(outOfBounds.getHeight() * scaleY) < 5)
			return;
		// check that scaling will not cause the element to go out of bounds using Rectangle2D?
		AffineTransform tx = new AffineTransform();
	...
	{
		double scaleVal;
		double d1 = Math.hypot(mouseX1 - mouseY1, centerX - centerY);
		double d2 = Math.hypot(mouseX2 - mouseY2, centerX - centerY);
		if (d1 == 0 || d2 == 0)
			scaleVal = 1;
		else
			scaleVal = d2 / d1;
	...
	public void translate (double deltaX, double deltaY)
	{
		...
		centerY += deltaY;
		AffineTransform tx = new AffineTransform();
		// centered at new centerX and centerY values that have added the delta X and Y 
		tx.translate(centerX, centerY);
		path.transform(tx);
	}
