...
public class ElementPanel extends JPanel
{
	...
	public ElementPanel (Data D)
	{
		...
		add (heart);
		// added my own shapes from E9
		OneElement leaf = new OneElement (data.newLeaf (0, 0, 100, 100, Color.BLACK, Color.WHITE));
		add (leaf);
		OneElement triangle = new OneElement (data.newTriangle (0, 0, 100, 100, Color.BLACK, Color.WHITE));
		add (triangle);
		for (int i = 0; i < 10; i++)
		{
			OneElement another = new OneElement (data.newRectangle (0, 0, 60, 60, Color.BLACK, Color.WHITE));
			add (another);
		}
	}
